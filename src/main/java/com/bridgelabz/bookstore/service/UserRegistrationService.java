...
public class UserRegistrationService implements IUserRegistrationService {
	...
	public ResponseDTO loginUser(LoginDTO logDto) {
		User  user=bookRepositroy.findByEmailId(logDto.getEmail()).orElseThrow(() -> new UserCustomException("login failed"));
	...
	public ResponseDTO updateUserById(Long id, UserDTO dto) {
		...
		if (isUserPresent.isPresent()) {
			...
			return new ResponseDTO("Update user sucesssfully",jwtToken.createToken(isUserPresent.get().getUserId()),200);
		} else {
	...
	public ResponseDTO forgotPwd(ForgotPasswordDTO forgotdto) {
		...
		if (isPresent.isPresent()) {
			...
			email.setBody(mailService.getLink("Hii  " +isPresent.get().getFirstName()+ " Reset your password -"+" http://localhost:8080/reset/", isPresent.get().getUserId()));
			mailService.send(email.getTo(), email.getSubject(), email.getBody());
